...
window.renderizarHabilidadesPersonagem = () => {
    ...
    p.habilidades.forEach((hab, index) => {
        ...
        // NOVA LÓGICA: Verifica se a habilidade tem um elemento e cria a tag
        let tagElemento = "";
        if (hab.elemento) {
            const classeElemento = hab.elemento.toLowerCase();
            tagElemento = `<span class="tag-elemento ${classeElemento}">${hab.elemento}</span>`;
        }
